-- 113 — Allow self/parent to set training_started_at (academy join date) via
-- edit_student_self. The import never carried this date and neither created_at
-- nor first attendance is the real join date (created_at = import day; first
-- attendance = when the app started logging). The only person who knows it is
-- the student/parent, so the field must be theirs to fill. Same CASE-WHEN shape
-- as bjj_start_date, already in the allowlist. Staff keep the direct-UPDATE path.
-- Body identical to migration in prod except the one training_started_at line.
...
AS $function$
DECLARE target_id uuid; result public.students;
BEGIN
  SELECT s.id INTO target_id FROM public.students s
